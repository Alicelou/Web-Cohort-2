...
function drawAceOfSpades() {
  let deckId = document.getElementById("deck").textContent;
  fetch(`https://deckofcardsapi.com/api/deck/${deckId}/draw/?count=52`) // Draw all cards
    .then((response) => response.json())
    .then((data) => {
      console.log(data);
      var hand = document.getElementById("hand");
      ...
      let aceOfSpades = data.cards.find((card) => card.code === "AS"); // Find Ace of Spades
      ...
      if (aceOfSpades) {
        ...
        console.error("Ace of Spades not found in this draw.");
      }
    })
    .catch((error) => {
      console.error("Error:", error);
